			...
			for myfrow in myfrows:
				...
				if resp=='0':
					...
					if resp not in resptypes:
						...
						if A1anno==A2anno:
							myfagrees+=1
							ftagrees.append(resp)
						else:
							myfdisagrees+=1
							ftdisagrees.append(resp)
...
for n in range(1,4):
	...
	for tu in tuversions:
		itemtuagreesfile=open('agreement_files/'+numstr+tu+'_type_agrees.csv', 'w')
		itemtuagreeswriter=csv.writer(itemtuagreesfile, dialect=csv.excel)
		itemtuagreeswriter.writerow(header)
		itemtudisagreesfile=open('agreement_files/'+numstr+tu+'_type_disagrees.csv', 'w')
...
for n in range(1,4):
	numstr=str(n).zfill(2)
	for feat in feats:
		itemfeatagreesfile=open('agreement_files/'+numstr+feat+'_type_agrees.csv', 'w')
		itemfeatagreeswriter=csv.writer(itemfeatagreesfile, dialect=csv.excel)
		itemfeatagreeswriter.writerow(header)
		itemfeatdisagreesfile=open('agreement_files/'+numstr+feat+'_type_disagrees.csv', 'w')
...
for n in range(1,4):
	numstr=str(n).zfill(2)
	itemfeatagreesfile=open('agreement_files/'+numstr+'_type_agrees.csv', 'w')
	itemfeatagreeswriter=csv.writer(itemfeatagreesfile, dialect=csv.excel)
	itemfeatagreeswriter.writerow(header)
	itemfeatdisagreesfile=open('agreement_files/'+numstr+'_type_disagrees.csv', 'w')
	itemfeatdisagreeswriter=csv.writer(itemfeatdisagreesfile, dialect=csv.excel)
	itemfeatdisagreeswriter.writerow(header)
	for fn in allfns:
		if numstr in fn:
