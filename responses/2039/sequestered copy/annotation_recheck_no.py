...
def get_allresponses(csv_in):
	allresponses = []
	with open(infilepath+csv_in, 'rU') as masterfile:
...
RespStringList = get_allresponses(inputfilename) ##this is a list of csv rows, where each row contains a response and an annotation; [['the boy threw the ball', '1'], ['the boy is throwing balls', '1'], ... ]
...
class App(Frame):
	def __init__(self,master):
		...
		#RESPONSE LABEL
		self.CRS = RespStringList[self.CurrentRespNum][0] ###Pay attention to this area (1/17/17)
		self.old_annot = RespStringList[self.CurrentRespNum][1] ###Pay attention ...
		if self.old_annot == '1': ## already annotated "yes" ("1")
			write_type_csv([self.CRS, self.old_annot])
		else:
			self.CurrentRespStringVar = StringVar()
			self.CurrentRespString = Label(self, height=4, width=65, font=tf, textvariable=self.CurrentRespStringVar, anchor='w', wraplength=500, justify='left', bg="bisque")
			self.CurrentRespString.grid(column=0,row=5,columnspan=1, sticky='EW')
