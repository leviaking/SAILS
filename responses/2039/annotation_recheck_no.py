...
###PRE-PROCESS; this section needs to sort "no" ("0") annotations from "yes" ("1") and "maybe" ("5") annotations. The "no" annotations should be written to a separate file, which will then be opened in a slightly modified version of the annotation interface.
...
for tirow in tempincsvreader:
	if tirow[1].strip()=='0': ## '0' indicates a "no" annotation
...
class App(Frame):
	def __init__(self,master):
		...
		#CHANGE ANNOTATION BUTTON
		self.ChangeButtonVar = StringVar()
		self.ChangeButton = Button(self, font=tf, width=20, highlightbackground="green", text='''Change "no" to "yes"''', command=self.ChangeKey)
		self.ChangeButton.grid(column=0,row=6, columnspan=1, padx=8, pady=8, sticky='W')
	
		#KEEP BUTTON
		self.KeepButtonVar = StringVar()
		self.KeepButton = Button(self, font=tf, width=20, highlightbackground='red', text='''Keep "no"''', command=self.KeepKey)
	...
	def ChangeKey(self, event=None): ##Change 'no' annotation to 'yes'
		self.annot = '1' ## Change 'O' to '1'
	...
	def KeepKey(self, event=None): ##Keep "no" annotation
		self.annot = '0' ## Keep '0' annotation
		#print 'ANNOTATION = '+self.annot
		write_type_csv(RespStringList[self.CurrentRespNum], self.annot)
		self.CurrentRespNum +=1
		try:
