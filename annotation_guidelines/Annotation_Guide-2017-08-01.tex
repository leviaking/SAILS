...
\paragraph{Content and composition} Interpretable responses are statements that could be illustrated with a canonical composition, without the need to infer any critical elements. A sentence that uses semantically empty references ("someone"/"something", etc.) for essential elements, or simply leaves them out, is not interpretable. Such a response could not be illustrated as a canonical painting, because some essential elements would have to be guessed or inferred. The response could, however, be represented as an abstract painting. 

It may be helpful for annotators to think of this as ``The Norman Rockwell Rule.'' That is, ``Would Norman Rockwell illustrate this response?'' Straightforward composition and a clear representational style are hallmarks of Rockwell's paintings. A response like ``The man is delivering a package to a woman'' fits this style of illustration. ``A man is delivering a package'' also fulfills the Rockwell Rule, because a painting of a delivery man leaving a package in a mailbox or on a doorstep could easily be imagined as a Rockwell painting. (Annotators should keep in mind that the image evoked by the response is not judged here for how well it matches the actual PDT image, and annotation should not be influenced by the PDT image.) For a response like ``Someone is delivering things to a woman,'' a Rockwell painting simply would not fit; both the deliverer and the thing being delivered would have to be out of frame, obscured, somehow abstracted, or purely guessed at. Annotators should rely on their own judgment when considering these content and composition concerns.

\paragraph{Unbound pronouns and ``something''/``someone'' arguments} \label{para:interp-arguments} A response that uses a nonspecific noun like ``something'' or ``someone'' or a pronoun ``it'', ``that'', ``they'' (without a specified or understood antecedent) as a necessary verb argument may be marked as interpretable if it nonetheless contains adequate content for illustration. \textit{The girl is singing something}, for example, is likely interpretable; \textit{singing} can be adequately represented without a specific object.
\lk{Not sure I'm satisfied with this...}
Annotators may disagree, however, on the adequacy of responses like \textit{The girl is building something} and \textit{The girl is building it}. Here annotators will need to decide for themselves whether a given verb with a nonspecific argument is adequately descriptive.
