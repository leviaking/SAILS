...
In short, my previous approach tried to assess each NNS response by extracting a \textit{verb(subject,object)} triple and trying to match it with a triple from the NS responses. This involved dependency parsing the sentence then applying custom rules based on the labels, relationships and parts of speech in order to find each element of the triple. This process found moderate success in correctly assessing a large portion \lk{really a majority?} of NNS responses with a very small number of NS responses. Considerable weaknesses emerged, however; the rule based approach meant that it was limited in its ability to handle variation, and the use of simple triples was a hacky simplification of meaning.

\lk{above P was added later; sync it up with following P}
