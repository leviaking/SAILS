...
%\subsection{Obtaining a syntactic form}
...
To illustrate this process, consider (\ref{ex:F2-type}).  We pass this
sentence through the parser to obtain the dependency parse shown in
Figure~\ref{fig:F2-dependency}. The parsed sentence then moves to the
...
(expletive) label; having none, it moves rightward to the {\tt
  nsubjpass} (noun subject, passive) node. Because we find an {\tt
  nsubjpass} label, the sentence moves leftward to the {\tt agent}
node. This label is also found, thereby reaching a terminal node and
being labeled as a type F2 sentence.
...
Before a more thorough analysis, we look at the distribution of
