...
In this section, I revisit the five annotation features discussed in Chapter~\ref{chap:annotation} to see how they correlate with the performance of my system. For a given feature, I use mean average precision (MAP) to see how well the system rankings separate the positively and negatively annotated responses. In other words, for the purposes of  calculating MAP for the \feat{core e\-vent} feature, a ``0'' annotation for \feat{core e\-vent} is treated as an error. As discussed in Section~\ref{sec:metrics}, average precision is a measure of how well a ranking separates errors from non-errors. To get average precision, one calculates the precision of error detection at every point in the ranking, then averages these values. I refer to \textit{mean} average precision here because the average precision values for all 30 items are then averaged for the purpose of evaluating the system configuration that produced the rankings.
