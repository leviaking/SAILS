...
\hline
4 & kicking & kick & VBG & 0 (\textit{root}) & root \\
\hline
...
\hline
\hline
    \multicolumn{6}{|c|}{\begin{dependency}[arc edge,text only label,label style={above}]
    \begin{deptext}[column sep=.5em]
      ROOT \& DET \&[1em] NN \& VBZ \&[1em] VBG \& DET \&[1em] NN \\
      \textit{root} \& The \&[1em] boy \& is \&[1em] kicking \& the \&[1em] ball \\
    \end{deptext}
...
%    \depedge[edge style={dotted}]{3}{2}{det}
  ...
  \end{dependency}} \\
\hline
\multicolumn{3}{|l|}{Sentence type: \textit{A} (see Table~\ref{tab:sentence-type})} & \multicolumn{3}{|l|}{Extracted triple: \textit{kick(boy,ball)}} \\
\hline
\end{tabular}
\end{center}
...
Determining the sentence type was accomplished by arranging a small set of binary decisions into a tree, as shown in Figure~\ref{fig:decision-tree}. This decision tree checks for the presence of various dependency labels. The extraction rules for the particular sentence type were then applied to obtain the semantic triple. 

\lk{change this example to the one used in 3.3}
\lk{add a sentence explaining how sentence types are connected to ``templates'' that allow us to ID the triple parts}
To illustrate this extraction process, consider Figure~\ref{fig:conll}.  I pass this
...
node. This label is found, so the sentence is passed leftward to the {\tt nsubj} node. This label is found, so the sentence is passed leftward, to the the terminal node that labels it as a type A sentence, as shown in Table~\ref{tab:sentence-type}. The rules for type A sentences dictate that the verb, subject and direct object are the dependents with labels {\tt root}, {\tt nsubj}, and {\tt dobj} respectively, so the system extracts \textit{kicking(boy, ball)} as the triple for this sentence.
%
%This label is not found, so the sentence is passed rightward to the {\tt nsubj} (noun subject) node. This label is found, so the sentence is passed leftward to the {\tt prep\_\**} node. The {\tt prep\_with} label is found, so the sentence is passed leftward to the terminal node that labels it as a type B sentence, as shown in Table~\ref{tab:sentence-type}.

Finally, for each NNS response, the resulting triple was lemmatized, with \textit{kicking(boy, ball)}, for example, changed to \textit{kick(boy, ball)}. This lemmatized NNS triple was then checked against the list of lemmatized NS triples. Ideally, each acceptable response would find a match, and unacceptable responses would not.
...
In many natural language processing scenarios where high dimensional vectors are involved, such as sentence encoders or word embeddings, machine learning and methods for dimensionality reduction are employed \citep{BertDevlin2018,word2vec}. This improves efficiency by reducing the storage and computing power needed. In my FC and TC approaches, however, the number of term types remained small enough that the raw vectors representing dependencies' tf-idf scores can be processed easily with an ordinary PC. Not only does this simplify the task, it means that the process remains transparent. There are no hidden processes or unexplainable representations.  If desired, each sentence vector can be examined value by value, where each number maps to a real syntactic dependency. This is important because it leaves the door open for meaningful feedback on each response. For example, one might choose to identify the most salient dependencies in the NS model and use them to guide an ICALL user from a low scoring response to a better response. This is not to argue that machine learning NLP tools are never suitable for use in ICALL, however. I relied on SBERT as a benchmark throughout the main study (see Sections~\ref{sec:exp-annotations} and~\ref{sec:exp-holistic}) and discuss the potential use of hybrid transparent/machine learning approaches to ICALL in Section~\ref{sec:optimization-conclusion} \cite{sbert2020}. 
...
Combining the various parameter settings results in 30 different configurations, and an experiment was run with each (section~\ref{sec:parameters}). For example, one such configuration is $[$\param{ldh}, \param{TC}, \param{WSJ}$]$.
...
\param{Brown} appears among top scoring configurations more often than does \param{WSJ}. Finally, for the term representation, labeled (\param{ldh}) and unlabeled (\param{xdh}) dependencies are used most among the top scoring configurations.

I also summarize the rankings for each isolated parameter, presented in Table~\ref{tab:dist-ranked-parameters}. For a given parameter, e.g., \param{ldh}, I averaged the scores from all configurations including \param{ldh} across all 10 items. Generally, the same trends appear salient. Notably, \param{TC} outperformed the other approaches, with \param{FC} and \param{TA} close behind (and nearly tied). Performance fell for the simplest approach, \param{FA}, which was in fact intended as a kind of baseline. With \param{\textbf{T}C}$>$\param{\textbf{F}C} and \param{\textbf{T}A}$>$\param{\textbf{F}A}, tf-idf weighting seems preferable to basic frequencies. Likewise, with \param{T\textbf{C}}$>$\param{T\textbf{A}} and \param{F\textbf{C}}$>$\param{F\textbf{A}}, for my term based scoring, taking the cosine of  score vectors outperformed simply comparing score averages.
