...
##need to make sure that entries with identical partcipant ID # also have identical demographic info, so I'm copying from Untarg (manually corrected) to replace the Targ (uncorrected)

ufilename='untargdemo.csv'
tfilename='targdemo.csv'
ufile=open(ufilename, 'r')
tfile=open(tfilename, 'r')
newfilename='newtarg.csv'
...
skipuheader=next(ureader, None)
skiptheader=next(treader, None)
...
for tri in trindices:
	if tri in urdict:
		newrows.append(urdict[tri])
	else:
		newrows.append(trdict[tri])

for nr in newrows:
	newwriter.writerow(nr)
...
# 
# FeatDict = {
#     'T_Gramm': 'Does the response below meet the criteria for GRAMMATICALITY?',
#     'U_Gramm': 'Does the response below meet the criteria for GRAMMATICALITY?',
...
# 
# TargFeats = ['T_Answer', 'T_Core', 'T_Interp', 'T_Verif', 'T_Gramm']
# NoImgTargFeats = ['T_Interp']
# UntargFeats = ['U_Answer', 'U_Core', 'U_Interp', 'U_Verif', 'U_Gramm']
