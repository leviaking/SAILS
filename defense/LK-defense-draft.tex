...
\begin{itemize}
\item Collected 13,533 PDT responses from 499 participants;
\vspace{.4em}
\item Annotated for 5 features, focused on content;
\vspace{.4em}
\item Established feature weights and benchmark rankings;
\begin{itemize}
\vspace{.4em}
\item Features and weights are reliable;
\end{itemize}
\vspace{.4em}
\item Developed explainable semantic textual similarity system based on dependencies and tf-idf;
\vspace{.4em}
\item Uncovered some exploitable patterns for predicting features and holistic quality;
\end{itemize}
...
\begin{itemize}
\item Explore results for broader range of L1s;
\vspace{.4em}
\item Compare results across L2 English proficiency levels;
\vspace{.4em}
\item Further map relationship between complexity and optimal settings;
\vspace{.4em}
\item For a given PDT item, try clustering responses into multiple models;
\begin{itemize}
\vspace{.4em}
\item Route NNS responses to most appropriate model;
\end{itemize}
\end{itemize}
\end{frame}
