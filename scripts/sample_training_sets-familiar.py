...
## Adapted to operate only on the familiar responses (which are far fewer);
## In this case we are not splitting r1 and r2 -- all sampled sets use both combined;
## I.e., for Familiar, we're doing only r2
...
def write_sets(hd, srcdir, srd):
	for k in srd:
		...
		if "gNSF" in k:
			if "r1" in k:
				# # # # r1sample = random.sample(srd[k],14)
				r2r1sample = random.sample(srd[k],7)
				r2k = k.replace("r1", "r2")
				r2r2sample = random.sample(srd[r2k],7)
				r2sample = r2r1sample+r2r2sample
				r2nm = srcdir+'F14/'+r2k+'-F14.csv'
