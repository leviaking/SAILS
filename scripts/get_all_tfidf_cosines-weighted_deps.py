...
def get_ref_terms_list(somedoc, d):
	terms = []
...
def build_ref_termlists():
	...
	for dt in dts:
		...
		for refdoc in myrefdocs:
			reftokens = get_ref_terms_list(refdoc, dt)
...
def get_training_tokens(gsrows, dty):
	gsts=[]
	for gsrow in gsrows:
...
def main():
	...
	dtwdict = build_ref_termlists()  ## Brown corpus terms for tf-idf
	for testfn in test_file_names:
		...
		for trainingfn in training_fns:
			...
			for deptype in ['ldh', 'xdh', 'xdx']:
				...
				dtw=dtwdict[deptype]
				training_tokens = get_training_tokens(curr_training_rows, deptype)
				model_tfidf_pairs = get_weighted_tfidf_pairs(dtw, training_tokens, deptype)  ## here, re weighting deps
				# print("WEIGHTED: ")
				# print(model_tfidf_pairs)
				# model_tfidf_pairs = get_tfidf_pairs(dtw, training_tokens, deptype)  ## here, re weighting deps
				# print("UNWEIGHTED: ")
				# print(model_tfidf_pairs)
				for test_row in curr_test_rows:
					participant_id = test_row[0][10:14]
					vectors_path_name = scored_vecs_dir+"term_vectors/"+training_label+"-VS-"+participant_id+"-W.csv"
			...
			while ni<len(curr_test_rows):
				...
				ni+=1
			outname=trainingfn.replace(".csv", "-VS-N70-W.csv")
