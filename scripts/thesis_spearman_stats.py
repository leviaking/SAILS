...
def from_experiment_get_stats_csvs(wd, exp, all_sources, sd):
	for param in exp:
		# print(all_sources)
		# print("param: "+param)
		...
		for setting in settings:
			...
			# print(ps_name)
			ps_df.to_csv(wd+ps_name+'_spearman.csv', encoding='utf-8', index=False, header=x_head)
			ps_sp_descriptive.to_csv(wd+ps_name+'_spearman_stats.csv', encoding='utf-8', header=['spearman'])
			ps_pv_descriptive.to_csv(wd+ps_name+'_sp_pval_stats.csv', encoding='utf-8', header=['p_value'])
		# param_col_zero = ['null', 'count', 'mean', 'std', 'min', '25%', '50%', '75%', 'max']
		p_descriptive = pandas.concat([pd for pd in param_stats], axis=1)
		p_descriptive.to_csv(wd+param+'_spearman_stats.csv', encoding='utf-8', header=param_header)
