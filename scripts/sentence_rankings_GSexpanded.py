...
##python sentence_rankings_GSexpanded.py NNSO_brown_tfidf GSexpanded_brown_tfidf
##python sentence_rankings_GSexpanded.py NNSO_wsj_tfidf GSexpanded_wsj_tfidf
##python sentence_rankings_GSexpanded.py NNSLM_brown_tfidf GSexpanded_brown_tfidf
##python sentence_rankings_GSexpanded.py NNSLM_wsj_tfidf GSexpanded_wsj_tfidf
...
def get_col_pairs_for_all_participants(td, ti, g, f, md):
	...
	refcorp = g.split('_')[1]
	goldvectorarg = g+'/i'+ti+'gs.'+f+'_'+refcorp+'.tfidf' ##'GSexpanded_wsj_tfidf/i01gs.ldh_wsj.tfidf
...
def get_baseline_columns_lemma(td, ti, md): 
	col_pairs = []
	tsource = td.split('_')[0].strip()
	goldtextin = open('GSexpanded/lemma_conll/i'+ti+'gs.lemma_conll', 'r') ##'GSexpanded/lemma_conll/i01gs.lemma_conll'
...
def get_baseline_columns_depstrings(td, ti, f, md):
	col_pairs = []
	tsource = td.split('_')[0]
	goldtextin = open('GSexpanded/'+f+'/i'+ti+'gs.'+f, 'r') ##'GSexpanded/ldh/i01gs.ldh'
	goldlines = goldtextin.readlines()
	goldtextin.close()
	goldbag = []
	for gl in goldlines:
...
def main():
	if not os.path.exists('exp_csvGSexpanded'):
		os.makedirs('exp_csvGSexpanded')
	...
	for testitem in testitems: ##this loop gets a sorted list of tuples (cosine, sentence) representing a pair of csv columns, for each set of test parameters.
		...
		lemma_csv = open('exp_csvGSexpanded/'+lemma_csv_name+'.csv', 'w')
		...
		clem_csv = open('exp_csvGSexpanded/'+clem_csv_name+'.csv', 'w')
		...
		for form in forms:
			...
			base_csv = open('exp_csvGSexpanded/'+base_csv_name+'.csv', 'w')
			...
			charlie_csv = open('exp_csvGSexpanded/'+charlie_csv_name+'.csv', 'w')
			...
			exp_csv = open('exp_csvGSexpanded/'+exp_csv_name+'.csv', 'w')
			...
			a_csv = open('exp_csvGSexpanded/'+a_csv_name+'.csv', 'w')
